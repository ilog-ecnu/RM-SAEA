classdef RMPBNSGAIII < ALGORITHM
        ...
        function main(Algorithm,Problem)
           ...
           [z,znad]      = deal(min(Population.objs),max(Population.objs));
            while Algorithm.NotTerminated(A)
                ...
                while w <= wmax
                    w = w + 1;
                    if w <= (1-(Problem.FE-50)/(Problem.maxFE*2))*wmax
                        OffspringDec = OperatorGA(Problem,Dec(randi(end,1,NI),:));
                    else
                        [FrontNo,MaxFNo] = NDSort(Obj,5);
                         Next = FrontNo <= MaxFNo;
%                         Last   = find(FrontNo==MaxFNo);
%                         Choose = LastSelection(Obj(Next,:),Obj(Last,:),5-sum(Next),W,Zmin); 
%                         Next(Last(Choose)) = true;
                        Dec1 = Dec(Next,:);

                        OffspringDec = Operator(Dec1,Problem.M,Problem);
                    end
               ...
               %% Pareto-based bi-indicator infill sampling criterion 
                ...
                A2   = [Population,New];
                Zmin = min(A2.objs,[],1);
                [Population,z,znad] = EnvironmentalSelection([Population,New],W,Problem.N,z,znad);
            end
        end
    end
end